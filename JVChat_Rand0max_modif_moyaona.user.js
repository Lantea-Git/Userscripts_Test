// ==UserScript==
// @name         JVChat Premium FORK by Rand0max_modif_moyaona
...
// --- DÉBUT DE LA MODIFICATION ---
    // On définit une hauteur fixe pour la zone de texte et on active la barre de défilement verticale
    GM_addStyle(`
        .jvchat-textarea.jvchat-textarea-focus,
        .jvchat-textarea {
            height: 80px !important; /* Définissez ici la hauteur fixe que vous voulez */
            resize: none !important; /* Empêche l'utilisateur de redimensionner manuellement */
            overflow-y: auto !important; /* Affiche une barre de défilement si le texte dépasse */
        }
    `);
...
async function postJvcMessage() {
    ...
    try {
        ...
        setScrollDown();
         // --- DÉBUT DE LA MODIFICATION ---
        // On force le défilement vers le bas APRÈS un court délai
        // pour laisser le temps à forceUpdate() de récupérer et d'afficher le nouveau message.
        setTimeout(setScrollDown, 1100); // 1100ms, juste après le forceUpdate
        // --- FIN DE LA MODIFICATION ---
...
function triggerJVChat() {
    ...
    dispatchEvent(event);
        ...
        // --- DÉBUT DE LA MODIFICATION : Ajout du bouton pour masquer/afficher le formulaire ---
    const formBlock = document.getElementById('bloc-formulaire-forum');
    if (formBlock) {
        ...
        `;
        // Insertion du bouton juste avant le formulaire
        formBlock.insertAdjacentHTML('beforebegin', toggleButtonHtml);

        // Récupération du bouton fraîchement créé
        const toggleButton = document.getElementById('jvchat-toggle-form-btn');

        // Ajout de la logique au clic
        toggleButton.addEventListener('click', (e) => {
            e.preventDefault();

            // On bascule la visibilité du formulaire
            formBlock.classList.toggle('jvchat-hide');

            // NOUVEAU : On vérifie si le formulaire est maintenant caché pour changer l'icône
            const isHidden = formBlock.classList.contains('jvchat-hide');

            if (isHidden) {
                // Si le formulaire est caché, on affiche la flèche vers le haut
